...
func LoadConfig(self *Authority) {
	// 读取 YAML 文件
	data, err := os.ReadFile(self.ConfigPath)
	if err != nil {
	...
	self.Configs.GlobalPermissionFirst = config.GlobalPermissionFirst
	self.Configs.PermissionLevel = config.PermissionLevel
	self.Configs.Authorites = convert(config.Authorites).(map[string]interface{})
}

func GetPermissionLevel(self *Authority, sourceID []string) (bool, int) {
	permissionLevelNotNone := false
	var permissionLevel int
	authorities := self.Configs.Authorites
	idIndex := 0
	...
	for {
		...
		if _, ok := authorities["GLOBAL"]; ok && backtrackingTimes <= 0 {
			if _, ok := authorities[xID]; !ok && self.Configs.GlobalPermissionFirst {
...
func GetPermissionByLevel(self *Authority, level int) (bool, string) {
	for permission, l := range self.Configs.PermissionLevel {
		if l == level {
...
func CheckCommandPermission(self *Authority, command string, sourceID []string) (bool, bool) {
	permissionLevelNotNone, permissionLevel := GetPermissionLevel(self, sourceID)
	permissionNeed := "USER" //TODO 等待FuncDict的接口实现后补全 func_dict.get_permission(command)
	if permissionLevelNeed, ok := self.Configs.PermissionLevel[permissionNeed]; ok {
